...
class HMM(object):
    def __init__(self, tag2idx, idx2tag):
        ...
        self.n_tag = len(self.tag2idx) # 21

        self.n_char = 65536  # 字符数
        self.epsilon = 1e-100  # 无穷小量，防止归一化时分母为0

        # 状态转移矩阵A：A[i][j]表示从i状态转移到j状态的概率
        self.A = np.zeros((self.n_tag, self.n_tag))  # [21, 21] 状态转移概率矩阵
        # 观测概率矩阵B：B[i][j]表示i状态下生成j字符
        self.B = np.zeros((self.n_tag, self.n_char))  # [21, 65536] 观测概率矩阵
        # 初始状态概率pi：pi[i]表示初始时刻为i状态的概率
        self.pi = np.zeros(self.n_tag)  # [21,] 初始隐状态概率
    ...
    def viterbi(self, string):
        # 初始化delta矩阵，delta[t][i]表示t时刻状态为i的最大概率
        delta = np.zeros((len(string), self.n_tag))
        # 初始化psi矩阵，psi[t][i]表示t时刻状态为i的最大概率路径的前一个状态
        psi = np.zeros((len(string), self.n_tag), dtype=int)
